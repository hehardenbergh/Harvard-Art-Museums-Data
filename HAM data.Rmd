...
tidydata <- raw %>%
  ...
  mutate(total_moves = sum(moves_2009 + moves_2010 + moves_2011 + moves_2012 + moves_2013 + moves_2014 + moves_2015 + moves_2016 + moves_2017 + moves_2018, na.rm = TRUE)) %>%  
...
tidydata %>% 
  ggplot(aes(accessionyear, total_moves, color=personculture), + geom_point())

```

```{r write rds for shiny app}
write_rds(tidydata, path = "shiny_data.rds")
```
